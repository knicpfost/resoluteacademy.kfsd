<?php
		/*
		Plugin Name: PayPal Payment Terminal Wordpress
		Plugin URI: http://www.convergine.com
		Description: Plugin for easily accepting paypal payments or donations on your blog
		...
		*/
		...
		function pppt_install(){
			...
			$table = $wpdb->prefix."pppt_services";
    		$structure = "CREATE TABLE IF NOT EXISTS $table (
						...
						pppt_services_descr MEDIUMTEXT NULL,
			...
			update_option('pppt_ty_text',"<p>Thank you for your payment! We really appreciate it.</p>");
		...
		function pppt_admin_actions() {
			global $wp_url;
			add_menu_page( "PayPal Terminal", "PayPal Terminal", "administrator", basename(__file__), "pppt_admin_overview", $wp_url."/wp-content/plugins/paypal_payment_terminal/resources/images/paypal.png");
			//add_menu_page( "PayPal Terminal Settings", "PayPal Terminal", "administrator", basename(__file__), "pppt_display_admin_menu", "icon URL");
			//add_options_page("PayPal Terminal Settings", "PayPal Terminal Settings", 1, "PayPal Terminal Settings", "pppt_admin");
			add_submenu_page( basename(__file__), 'PayPal Terminal Settings', 'Settings', 'administrator', 'pppt_admin_settings','pppt_admin_settings');
			add_submenu_page( basename(__file__), 'PayPal Terminal Services', 'Services', 'administrator', 'pppt_admin_services','pppt_admin_services');
			add_submenu_page( basename(__file__), 'PayPal Terminal Transactions', 'Transactions', 'administrator', 'pppt_admin_transactions','pppt_admin_transactions');
			add_submenu_page( basename(__file__), 'PayPal Terminal Service Edit', '', 'administrator', 'pppt_admin_services_edit','pppt_admin_services_edit');
		...
		function pppt_admin_overview() { /* plugin overview page */
			include('paypal_terminal_overview.php');
		}

		function pppt_admin_settings() { /* settings page */ 
			include('paypal_terminal_settings.php');
		}
		function pppt_admin_services() { /* services page */
			include('paypal_terminal_services.php');
		}
		function pppt_admin_services_edit() { /* services editing page */
			include('paypal_terminal_services_edit.php');
		}
		function pppt_admin_transactions() { /* transactions page */
			include('paypal_terminal_transactions.php');
		}
		...
		class pppt_widget extends WP_Widget {
				...
				function pppt_widget(){
					/* Widget settings. */
					$widget_ops = array( 'classname' => 'pppt_widget', 'description' => __('Widget allows you to place paypal payment terminal form in a sidebar of your wordpress.', 'pppt_widget') );
					/* Widget control settings. */
					$control_ops = array( 'width' => 250, 'height' => 200, 'id_base' => 'pppt_widget' );
					/* Create the widget. */
					$this->WP_Widget( 'pppt_widget', __('PayPal Payment Terminal', 'pppt_widget'), $widget_ops, $control_ops );
				}
				...
				function widget( $args, $instance ) {
					...
					*/
					...
					if($pppt_show_form==1){
					if ( $title )
						echo $before_title . "Pay by <img src='".$wp_url."/wp-content/plugins/paypal_payment_terminal/resources/images/paypal_logo.png"."' width='70'>" . $after_title;
					}
			
					$pppt_wform = '<link rel="stylesheet" type="text/css" media="all" href="'.$wp_url.'/wp-content/plugins/paypal_payment_terminal/resources/css/pppt-widget.css" />';
					...
					if($pppt_show_form==1){
						...
						if($pppt_show_dd_text==1){
							...
							$result=mysql_query($query) or die(mysql_error());
							if(mysql_num_rows($result)>0){
								$pppt_wform .= '<li class="ppptTitle">Service:</li><li><select name="pppt_serviceID" id="pppt_serviceID">';
								$pppt_wform .= '<option value="">Please Select</option>';
								while($row=mysql_fetch_assoc($result)){
									if($pppt_show_am_text=="1"){
										$pppt_wform .= '<option value="'.$row["pppt_services_id"].'">'.stripslashes($row["pppt_services_title"]).'</option>';
									} else { 
										$pppt_wform .= '<option value="'.$row["pppt_services_id"].'">'.stripslashes($row["pppt_services_title"]).' - '.number_format($row["pppt_services_price"],2).'</option>';
									}
								}
								$pppt_wform .= '</select>';
						...
						$pppt_wform .= '</ul>';
						$pppt_wform .= '<a href="http://www.convergine.com">'.$copyright_img.'</a>';
						$pppt_wform .= '</form></div>';
					}
				...
				function form( $instance ) {
					...
					$defaults = array( 'title' => 'PayPal Payment Terminal Widget');
